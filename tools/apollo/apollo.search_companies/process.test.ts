...
const shimPath = path.resolve(
  repoRoot,
  "workflows/sales_prospect_research/tools/apollo.search_companies/run",
);
...
test("tool executable returns the standard envelope over stdin/stdout", {
  ...
  const result = await runJsonTool(
    ...
    { keywords: "developer tools", page: 1, perPage: 1 },
    {
      ...process.env,
      APOLLO_API_KEY: "test-key",
      EXPECTED_APOLLO_COMPANY_KEYWORDS: "developer tools",
      EXPECTED_APOLLO_COMPANY_PAGE: "1",
      EXPECTED_APOLLO_COMPANY_PER_PAGE: "1",
    },
  );
