...
export async function exportInstantlyCampaign(
  ...
  const payload = await instantlyRequest(
    {
      path: `/campaigns/${input.campaignId}/export`,
      method: "POST",
...
const onError = (error: unknown): ToolErrorInfo => {
  ...
  return {
    type: "external_error",
    message: error instanceof Error ? error.message : "Unknown Instantly campaign export error",
  };
};

export const instantlyCampaignExportTool = defineTool({
  name: "instantly.campaign.export",
  ...
  async handler({ input }) {
    return exportInstantlyCampaign(input);
  },
  onError,
});

if (import.meta.main) {
  void runDeclaredTool(instantlyCampaignExportTool);
}
