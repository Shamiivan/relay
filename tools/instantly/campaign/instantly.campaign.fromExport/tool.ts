...
export async function createInstantlyCampaignFromExport(
  ...
  const payload = await instantlyRequest(
    {
      path: `/campaigns/${input.campaignId}/from-export`,
      method: "POST",
...
const onError = (error: unknown): ToolErrorInfo => {
  ...
  return {
    type: "external_error",
    message: error instanceof Error ? error.message : "Unknown Instantly campaign from-export error",
  };
};

export const instantlyCampaignFromExportTool = defineTool({
  name: "instantly.campaign.fromExport",
  ...
  async handler({ input }) {
    return createInstantlyCampaignFromExport(input);
  },
  onError,
});

if (import.meta.main) {
  void runDeclaredTool(instantlyCampaignFromExportTool);
}
