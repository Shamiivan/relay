...
import { getInstantlyApiKey, getInstantlyBaseUrl, type InstantlyFetch } from "../../lib/client.ts";
...
export async function deleteInstantlyCampaign(
...
): Promise<{ deleted: true; campaignId: string }> {
  ...
  const url = new URL(`/api/v2/campaigns/${input.campaignId}`.replace("/api/v2/", ""), getInstantlyBaseUrl(env));
  ...
  return { deleted: true, campaignId: input.campaignId };
}
...
const onError = (error: unknown): ToolErrorInfo => {
  ...
  return { type: "external_error", message: error instanceof Error ? error.message : "Unknown Instantly campaign delete error" };
};

export const instantlyCampaignDeleteTool = defineTool({
  name: "instantly.campaign.delete",
  resource: "instantly.campaign",
  capability: "delete",
  description: "Delete an Instantly campaign by ID.",
  destructive: true,
  input: inputSchema,
  output: z.object({ deleted: z.literal(true), campaignId: z.string().uuid() }),
  prompt: promptFile("./prompt.md"),
  async handler({ input }) {
    return deleteInstantlyCampaign(input);
  },
  onError,
});

if (import.meta.main) {
  void runDeclaredTool(instantlyCampaignDeleteTool);
}
